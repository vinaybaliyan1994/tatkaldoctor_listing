...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-slate-800">Master Services</h1>
        <p class="text-slate-500 text-sm mt-1">Medical service categories and sub-services.</p>
    </div>
...
<form method="GET" action="{{ route('master-services.index') }}"
    ...
    <div class="flex-1 min-w-48">
        <label class="block text-xs font-medium text-slate-500 mb-1">Category / Parent</label>
    ...
    @if ($services->isEmpty())
        <div class="text-center py-16 text-slate-400">
            ...
            <p class="text-sm">No services found.
                @if (Auth::user()->isSuperAdmin())
                    <a href="{{ route('master-services.create') }}" class="text-teal-600 hover:underline">Add the first one.</a>
                @endif
            </p>
        </div>
    @else
        <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    ...
                    <th class="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Service Name</th>
                    <th class="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Type</th>
                    <th class="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Parent</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-6 py-3.5 text-slate-400 text-xs">{{ $svc->id }}</td>
                    <td class="px-6 py-3.5 font-medium text-slate-800">{{ $svc->service }}</td>
                    ...
                    <td class="px-6 py-3.5 text-slate-600 text-sm">
