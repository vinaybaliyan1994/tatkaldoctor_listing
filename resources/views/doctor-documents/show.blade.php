...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-2 text-sm text-slate-500">
        ...
        <span class="text-slate-800 font-medium">{{ $doctorDocument->document_type_label }}</span>
    </div>
    <div class="flex items-center gap-2">
        ...
        <a href="{{ route('doctor-documents.download', $doctorDocument) }}"
           class="px-4 py-2 bg-slate-700 hover:bg-slate-800 text-white text-sm font-medium rounded-xl transition-colors">
            Download
        </a>
        @endif
    </div>
...
<div class="max-w-2xl space-y-5">
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
        <div class="flex items-start justify-between">
            <div>
                <h1 class="text-xl font-bold text-slate-800">{{ $doctorDocument->document_type_label }}</h1>
                <p class="text-sm text-slate-500 mt-1">{{ $doctorDocument->listing->name }}</p>
            </div>
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
        <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide mb-4">File Details</h2>
        <dl class="space-y-3 text-sm">
            <div class="flex gap-3">
                <dt class="w-36 text-slate-500 flex-shrink-0">Original Name</dt>
                <dd class="text-slate-800">{{ $doctorDocument->original_name ?? '—' }}</dd>
            </div>
            <div class="flex gap-3">
                <dt class="w-36 text-slate-500 flex-shrink-0">MIME Type</dt>
                <dd class="text-slate-600 font-mono text-xs">{{ $doctorDocument->mime_type ?? '—' }}</dd>
            </div>
            <div class="flex gap-3">
                <dt class="w-36 text-slate-500 flex-shrink-0">File Size</dt>
                <dd class="text-slate-800">
                    {{ $doctorDocument->file_size ? number_format($doctorDocument->file_size / 1024, 1) . ' KB' : '—' }}
                </dd>
            </div>
            <div class="flex gap-3">
                <dt class="w-36 text-slate-500 flex-shrink-0">Uploaded</dt>
                <dd class="text-slate-800">{{ $doctorDocument->created_at->format('d M Y, H:i') }}</dd>
            </div>
        </dl>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
        ...
        <dl class="space-y-3 text-sm">
            ...
            <div class="flex gap-3">
                ...
                <dd class="text-slate-800">{{ $doctorDocument->verified_at->format('d M Y, H:i') }}</dd>
            </div>
            @endif
            @if ($doctorDocument->verifiedBy)
            <div class="flex gap-3">
                <dt class="w-36 text-slate-500 flex-shrink-0">Verified By</dt>
                <dd class="text-slate-800">{{ $doctorDocument->verifiedBy->name }}</dd>
            </div>
            @endif
            @if ($doctorDocument->remarks)
            <div class="flex gap-3">
                <dt class="w-36 text-slate-500 flex-shrink-0">Remarks</dt>
                <dd class="text-slate-700">{{ $doctorDocument->remarks }}</dd>
            </div>
            @endif
            @if (!$doctorDocument->verified_at && !$doctorDocument->remarks)
            <p class="text-slate-400">Not yet reviewed.</p>
            @endif
        </dl>
    </div>
    ...
    <div class="bg-white rounded-xl border border-red-100 shadow-sm p-6">
        ...
        <form method="POST" action="{{ route('doctor-documents.destroy', $doctorDocument) }}"
            ...
            @csrf @method('DELETE')
            <button type="submit"
                    class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-xl transition-colors">
                Delete Document
            </button>
        </form>
    </div>
