...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-gray-500 text-sm mt-1">Manage HMAC client credentials and access windows.</p>
    </div>
    ...
    @if ($clients->isEmpty())
        <div class="text-center py-16 text-gray-400">
            ...
            <p class="text-sm">No clients yet.
                @if (Auth::user()->isSuperAdmin())
                    <a href="{{ route('clients.create') }}" class="text-blue-600 hover:underline">Add the first one.</a>
                @endif
            </p>
        </div>
    @else
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">API Key</th>
                    <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Available From</th>
                    <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Available To</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4 text-gray-400 text-xs">{{ $client->id }}</td>
                    <td class="px-6 py-4 font-medium text-gray-800">{{ $client->name }}</td>
                    <td class="px-6 py-4">
                        <code class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded font-mono">{{ $client->api_key }}</code>
                    </td>
                    <td class="px-6 py-4 text-gray-600">{{ $client->avail_from_date?->format('d M Y') ?? '—' }}</td>
                    <td class="px-6 py-4 text-gray-600">{{ $client->avail_to_date?->format('d M Y') ?? '—' }}</td>
                    <td class="px-6 py-4">
