            ...
            <div class="space-y-3">
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <code id="api-key-val" class="flex-1 text-xs bg-white border border-amber-200 rounded px-3 py-2 font-mono text-slate-800 break-all">{{ session('api_key') }}</code>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <code id="secret-key-val" class="flex-1 text-xs bg-white border border-amber-200 rounded px-3 py-2 font-mono text-slate-800 break-all">{{ session('secret_key') }}</code>
...
<div class="max-w-2xl space-y-5">
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h2 class="font-semibold text-slate-800">Client Details</h2>
        ...
        <dl class="divide-y divide-slate-100">
            ...
            <div class="px-6 py-3.5 flex items-center gap-4">
                ...
                <dd class="text-sm text-slate-800 font-mono">{{ $row['value'] }}</dd>
            </div>
            @endforeach

            <div class="px-6 py-3.5 flex items-center gap-4">
                <dt class="w-36 flex-shrink-0 text-xs font-medium text-slate-500 uppercase tracking-wide">Status</dt>
                <dd>
            ...
            <div class="px-6 py-3.5 flex items-center gap-4">
                <dt class="w-36 flex-shrink-0 text-xs font-medium text-slate-500 uppercase tracking-wide">API Key</dt>
                <dd class="text-sm font-mono text-slate-800 break-all">{{ $client->api_key }}</dd>
            </div>

            <div class="px-6 py-3.5 flex items-center gap-4">
                <dt class="w-36 flex-shrink-0 text-xs font-medium text-slate-500 uppercase tracking-wide">Secret Key</dt>
                <dd class="text-sm text-slate-400 italic">Hidden — regenerate to issue a new one</dd>
            </div>
        </dl>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
        <h2 class="font-semibold text-slate-800 mb-1">Regenerate Keys</h2>
        ...
        <form method="POST" action="{{ route('clients.regenerate-keys', $client) }}"
            ...
            @csrf
            <button type="submit"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 text-sm font-medium rounded-xl transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
