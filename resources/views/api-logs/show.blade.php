...
<div class="mb-6 flex items-center gap-2 text-sm text-gray-500">
    <a href="{{ route('api-logs.index') }}" class="hover:text-gray-700">API Logs</a>
    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
    </svg>
    <span class="text-gray-800 font-medium">Log #{{ $apiLog->id }}</span>
</div>
...
<div class="max-w-4xl space-y-5">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h1 class="font-semibold text-gray-800">Request Summary</h1>
            @if ($apiLog->success)
        ...
        <dl class="divide-y divide-gray-100">
            @foreach ([
                ...
                <div class="px-6 py-3.5 flex gap-4">
                    ...
                    <dd class="text-sm text-gray-800 font-mono break-all">{{ $row['value'] }}</dd>
                </div>
            @endforeach

            <div class="px-6 py-3.5 flex gap-4">
                <dt class="w-36 flex-shrink-0 text-xs font-medium text-gray-500 uppercase tracking-wide">Error Message</dt>
                <dd class="text-sm {{ $apiLog->error_message ? 'text-red-700' : 'text-gray-400 italic' }}">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h2 class="font-semibold text-gray-800 mb-3">Request Headers</h2>
        <pre class="text-xs bg-gray-900 text-gray-100 rounded-lg p-4 overflow-x-auto">{{ json_encode($apiLog->request_headers, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</pre>
    </div>
</div>
