...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Doctor Listings</h1>
        <p class="text-gray-500 text-sm mt-1">All doctor profiles on the platform.</p>
    </div>
...
<form method="GET" action="{{ route('listings.index') }}"
    ...
    <div class="flex items-center gap-2">
        ...
        @if (request()->hasAny(['country_code', 'master_city_id', 'master_location_id', 'service_id', 'status', 'search', 'verification_status']))
    ...
    @if ($listings->isEmpty())
        <div class="text-center py-16 text-gray-400">
            ...
            <p class="text-sm">No listings found.
                @if (Auth::user()->isSuperAdmin())
                    <a href="{{ route('listings.create') }}" class="text-blue-600 hover:underline">Add the first one.</a>
                @endif
            </p>
        </div>
    @else
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Doctor Name</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Hospital Name</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">City</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Location</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Status</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Verification</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-4 py-3 text-gray-400 text-xs">{{ $listing->id }}</td>
                    <td class="px-4 py-3">
                        <p class="font-medium text-gray-800">{{ $listing->name }}</p>
                        @if (count($listing->qualification_names))
                            <p class="text-xs text-blue-600 mt-1">{{ implode(', ', $listing->qualification_names) }}</p>
                        @endif
                        @if (count($listing->service_names))
                            <p class="text-xs text-rose-600 mt-1">{{ implode(', ', $listing->service_names) }}</p>
                        @endif
                        <p class="text-xs text-gray-400 font-mono mt-0.5">{{ substr($listing->uuid, 0, 8) }}…</p>
                    </td>
                    <td class="px-4 py-3 text-gray-600">{{ $listing->hospital_name ?? '—' }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ $listing->city?->name ?? '—' }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ $listing->location?->location ?? '—' }}</td>
                    <td class="px-4 py-3">
                        @if ($listing->status)
