...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span class="text-gray-800 font-medium">{{ $subscriptionPlan->name }}</span>
    </div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-5">
    <div class="flex items-start justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-gray-400 font-mono mt-1">{{ $subscriptionPlan->slug }}</p>
            @if ($subscriptionPlan->description)
            <p class="text-gray-600 text-sm mt-2">{{ $subscriptionPlan->description }}</p>
            @endif
        </div>
        <div class="flex flex-col items-end gap-2">
            <div class="text-3xl font-extrabold text-purple-700">
                ...
                @else
                    ...
                    <span class="text-sm font-normal text-gray-400">/ {{ $subscriptionPlan->duration_days }} days</span>
                @endif
            </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Limits</h2>
        <dl class="space-y-3 text-sm">
            <div class="flex gap-3">
                <dt class="w-40 text-gray-500">Max Staff</dt>
                <dd class="font-medium text-gray-800">{{ $subscriptionPlan->max_staff ?? '∞ Unlimited' }}</dd>
            </div>
            <div class="flex gap-3">
                <dt class="w-40 text-gray-500">Max Locations</dt>
                <dd class="font-medium text-gray-800">{{ $subscriptionPlan->max_locations ?? '∞ Unlimited' }}</dd>
            </div>
            <div class="flex gap-3">
                <dt class="w-40 text-gray-500">Max Appointments</dt>
                <dd class="font-medium text-gray-800">{{ $subscriptionPlan->max_appointments ? $subscriptionPlan->max_appointments . '/month' : '∞ Unlimited' }}</dd>
            </div>
            <div class="flex gap-3">
                <dt class="w-40 text-gray-500">Duration</dt>
                <dd class="font-medium text-gray-800">{{ $subscriptionPlan->duration_days }} days</dd>
            </div>
        </dl>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Features</h2>
        ...
        @else
            <p class="text-gray-400 text-sm">No features listed.</p>
        @endif
    </div>

    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Subscribers</h2>
        <p class="text-3xl font-bold text-teal-700">{{ $subscriptionPlan->clientSubscriptions()->count() }}</p>
        <p class="text-xs text-gray-400 mt-1">total subscriptions</p>
