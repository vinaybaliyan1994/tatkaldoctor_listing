...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Master Countries</h1>
        <p class="text-gray-500 text-sm mt-1">ISO 3-letter country codes used across the platform.</p>
    </div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
    @if ($countries->isEmpty())
        <div class="text-center py-16 text-gray-400">
            ...
            <p class="text-sm">No countries yet. <a href="{{ route('master-countries.create') }}" class="text-blue-600 hover:underline">Add the first one.</a></p>
        </div>
    @else
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide w-32">Code</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-3.5">
                        <span class="inline-block px-2.5 py-0.5 bg-blue-50 text-blue-700 font-mono font-semibold text-xs rounded">
                            {{ $country->code }}
                        </span>
                    </td>
                    <td class="px-6 py-3.5 font-medium text-gray-800">{{ $country->name }}</td>
                    <td class="px-6 py-3.5 text-gray-500 text-xs">{{ $country->created_at->format('d M Y') }}</td>
