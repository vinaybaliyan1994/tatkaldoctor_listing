...
<div class="mb-6 flex items-center gap-2 text-sm text-gray-500">
    ...
    <a href="{{ route('client-subscriptions.show', $clientSubscription) }}" class="hover:text-gray-700">#{{ $clientSubscription->id }}</a>
    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
    </svg>
...
<h1 class="text-2xl font-bold text-gray-800 mb-1">Edit Subscription</h1>
<p class="text-gray-500 text-sm mb-6">Update subscription details for <strong>{{ $clientSubscription->client->name }}</strong>.</p>
        ...
        <div class="md:col-span-2">
            ...
            <textarea id="notes" name="notes" rows="3"
                      class="w-full px-4 py-2.5 rounded-lg border @error('notes') border-red-400 bg-red-50 @else border-gray-300 @enderror
                             text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 transition resize-none">{{ old('notes', $clientSubscription->notes) }}</textarea>
