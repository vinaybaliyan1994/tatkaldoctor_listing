...
<div class="mb-6 flex items-center gap-2 text-sm text-slate-500">
    ...
    <a href="{{ route('settings.show', $setting) }}" class="hover:text-slate-700 font-mono text-xs">{{ $setting->key }}</a>
    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
    </svg>
...
<h1 class="text-2xl font-bold text-slate-800 mb-1">Edit Setting</h1>
<p class="text-slate-500 text-sm mb-6 font-mono">{{ $setting->key }}</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div class="md:col-span-2">
            ...
            <textarea id="value" name="value" rows="4"
                      class="w-full px-4 py-2.5 rounded-xl border @error('value') border-red-400 bg-red-50 @else border-slate-200 bg-slate-50 @enderror text-sm text-slate-800 font-mono focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 transition resize-y">{{ old('value', $setting->value) }}</textarea>
